...
namespace AkkaMetrics.Collector.Actors {
  ...
  public class MetricsCollectorActor : ReceiveActor {
    ...
    static readonly ActivitySource ActivitySource = new("AkkaMetrics.Collector");
    ...
    public MetricsCollectorActor(string nodeId,
                                 string hubAddress) {
      this.nodeId = nodeId;
    ...
    void handleStartCollecting(StartCollecting msg) {
      using var activity = ActivitySource.StartActivity("StartCollecting");

      activity?.SetTag("node.id", nodeId);
      activity?.SetTag("interval.ms", msg.IntervalMs);

      if (isCollecting) {
        log.Warning("[{NodeId}] Already collecting metrics, ignoring start request", nodeId);
    ...
    void handleStopCollecting() {
      using var activity = ActivitySource.StartActivity("StopCollecting", ActivityKind.Internal);
      activity?.SetTag("node.id", nodeId);

      if (!isCollecting) {
        log.Info("[{NodeId}] Not collecting, ignoring stop request", nodeId);
    ...
    void handleCollectTick() {
      ...
      using var activity = ActivitySource.StartActivity("CollectMetrics", ActivityKind.Internal);
      activity?.SetTag("node.id", nodeId);
      activity?.SetTag("interval.ms", intervalMs);

      try {
          var cpu = getCpuPercent();
          getMemoryInfo(out double ramUsedMb, out double ramTotalMb);
          activity?.SetTag("cpu.percent", cpu);
          activity?.SetTag("ram.used.mb", ramUsedMb);
          activity?.SetTag("ram.total.mb", ramTotalMb);
          log.Info("[{NodeId}] cpu={C:F1}% ram={U:F0}/{T:F0}MB", nodeId, cpu, ramUsedMb, ramTotalMb);
          ...
          log.Error(ex, "[{NodeId}] tick error", nodeId);
...
#pragma warning restore CA1416
    ...
    static void readLinuxCpuCounters(out double idle,
                                     out double total) {
    ...
    static double getMacCpu() {
      try {
        ...
        foreach (var l in lines)
          if (l.TrimStart().StartsWith("CPU usage:"))
            cpuLine = l;
    ...
    static void getMemoryInfo(out double ramUsedMb,
                              out double ramTotalMb) {
      ramUsedMb = 0;
      ramTotalMb = 0;
    ...
    static void getLinuxMemory(out double ramUsedMb,
                               out double ramTotalMb) {
      ramUsedMb = 0;
      ramTotalMb = 0;
    ...
    static void getWindowsMemory(out double ramUsedMb,
                                 out double ramTotalMb) {
      ramUsedMb = 0;
      ramTotalMb = 0;
    ...
    static void getMacMemory(out double ramUsedMb,
                             out double ramTotalMb) {
      try {
        using var p = new Process {
